...
/**
 * 
 * A  Singleton Class ensure that there will only be 
 * ONE instance of this class during the runtime of an app
 *
 */
public class ConnectionUtil {
	...
	// The Connection INterface coemes from the java.sql package which is the JDBC API
	...
	public static Connection getConnection() {
		...
		// first we check if an instance of this class is already instantiated in the application
		try {
		...
		// To establish connection we need to read from a file with all the credentials to connect
		// to the database
		...
		try {
			// The properties object SUES a FileReader object oread all of values inside of the file
			prop.load(new FileReader("C:\\Users\\SophieGavrila\\Desktop\\demos\\2-persistence\\jdbc-demo\\src\\main\\resources\\application.properties"));
		
			url = prop.getProperty("url");
			username = prop.getProperty("username");
			password = prop.getProperty("password");
			
			// Once we  have the jdbc url, username, and password, we can establish a connection
